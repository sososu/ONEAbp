...
public class ProfileAppService : IdentityAppServiceBase, IProfileAppService
{
    ...
    public virtual async Task<ProfileDto> UploadAvatarAsync(UpdateAdvarInput input)
    {
        var currentUser = await UserManager.GetByIdAsync(CurrentUser.GetId());


        var result= await FileAppService.CreateAsync(new FileUploadInputDto { File = input.AvatarFile,Tag= "Avatar" });

        await UserManager.UpdateAvatar(currentUser, result.WebUrl);
