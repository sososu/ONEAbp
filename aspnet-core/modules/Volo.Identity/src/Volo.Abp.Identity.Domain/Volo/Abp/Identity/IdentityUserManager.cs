...
public class IdentityUserManager : UserManager<IdentityUser>, IDomainService
{
    ...
    public virtual async Task SetOrganizationUnitsAsync(IdentityUser user, params Guid[] organizationUnitIds)
    {
        ...
        // Add to added OUs Code
        var organizationUnits = await OrganizationUnitRepository.GetListAsync(organizationUnitIds);

        if(user.ExtraProperties.ContainsKey(nameof(IOrganizationCode.OrganizationCode)))
        {
            user.ExtraProperties[nameof(IOrganizationCode.OrganizationCode)] = organizationUnits.FirstOrDefault()?.Code ?? "";
        }
        else
        {
            user.ExtraProperties.Add(nameof(IOrganizationCode.OrganizationCode), organizationUnits.FirstOrDefault()?.Code ?? "");
        }
    ...
    public virtual async Task<IdentityResult> UpdateAvatar(IdentityUser user,string avatar)
    {
        user.Avatar = avatar;
        return await UpdateUserAsync(user);
    }
