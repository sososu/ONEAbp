...
{
    public class ONEInitialMigrationCreator : ITransientDependency
    {
        public ICmdHelper CmdHelper { get; }
        public DotnetEfToolManager DotnetEfToolManager { get; }
        public ILogger<InitialMigrationCreator> Logger { get; set; }

        public ONEInitialMigrationCreator(ICmdHelper cmdHelper, DotnetEfToolManager dotnetEfToolManager)
        {
            CmdHelper = cmdHelper;
        ...
        public async Task<bool> CreateAsync(string targetProjectFolder, bool layeredTemplate = true,IList<string>Dbcontexts=null)
        {
            ...
            if(tenantDbContextName.IsNullOrWhiteSpace() && Dbcontexts != null&& Dbcontexts.Count > 1)
            {
                bool AllmigrationSuccess=true;
                foreach (var dbContext in Dbcontexts)
                {
                    var migrationOutput = AddMigrationAndGetOutput(targetProjectFolder, dbContext, "Migrations");
                    ...
                    AllmigrationSuccess &= migrationSuccess;
                }

                return AllmigrationSuccess;
            }
            else
            {
        ...
        private string AddMigrationAndGetOutput(string dbMigrationsFolder, string dbContext, string outputDirectory)
        {
            ...
            var addMigrationCmd = $"dotnet ef migrations add {dbContext??string.Empty}Initial --output-dir {outputDirectory} {dbContextOption}";
