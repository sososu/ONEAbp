...
namespace TemplateBuilders
{
    ...
    public class ONEAbpIoSourceCodeStore : ISourceCodeStore, ITransientDependency
    {
        ...
        public ONEAbpIoSourceCodeStore(
            ...
            bool includePreReleases = false)
        {
            if (url == null)
            {
                ...
                url = $"{ONECliUrls.GetRealUrl(name)}api/download/{type}/get-version/";
            }
        ...
        private async Task<string> GetTemplateNugetVersionAsync(string name, string type, string version)
        {
            ...
            try
            {
                var url = $"{ONECliUrls.GetRealUrl(name)}api/download/{type}/get-nuget-version/";
                var client = _cliHttpClientFactory.CreateClient();
        ...
        {
            var url = $"{ONECliUrls.GetRealUrl(templateName)}api/download/all-versions?includePreReleases=true";
        ...
        {
            var url = $"{ONECliUrls.GetRealUrl(input.Name)}api/download/{input.Type}/";
